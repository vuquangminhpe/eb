...
const Coupon = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      fetch(`${API_URL}/products/seller/${JSON.parse(user).id}`, {
        method: "GET",
      })
        .then((res) => res.json())
        .then((data) => setProducts(data));
      fetch(`${API_URL}/coupons/seller/${JSON.parse(user).id}`, {
        method: "GET",
      })
        .then((res) => res.json())
        .then((data) => setCoupons(data));
            ...
            <tr key={coupon._id}>
              ...
              <td style={{ border: "1px solid #ddd", padding: "8px" }}>
                <button
                  ...
                  onClick={() => (setModalOpen(true), setChosenCoupon(coupon))}
                >
                  Edit
                </button>
                <button
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">
              {chosenCoupon ? "Edit Coupon" : "Add New Coupon"}
            </h3>
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex justify-end space-x-2">
                <button
                  type="button"
                  onClick={() => (setModalOpen(false), setChosenCoupon(null))}
